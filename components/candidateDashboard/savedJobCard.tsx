...
import { getSavedJobsByCandidateId } from "@/services/candidateDashobardApi";
...
export function SavedJobs({ limit }: SavedJobsProps) {
  ...
  useEffect(() => {
    ...
    if (id) {
      ...
      setError("You are not logged in. Please log in to view saved jobs.");
    }
  }, []);
    ...
    async (jobId: string) => {
      ...
      try {
        if (!isSaved) {
          ...
          setJobs((prev) => prev.filter((job) => job.id !== jobId)); // Remove unsaved job from list
        }
      } catch (error: any) {
  ...
  const handleJobApplied = useCallback((jobId: string) => {
    setJobs((prevJobs) => prevJobs.filter((job) => job.id !== jobId)); // Remove applied job from list
  }, []);
  ...
  const fetchJobs = useCallback(
    async (pageNum: number, reset: boolean = false) => {
      ...
      setLoading(true);
      try {
        const response = await getSavedJobsByCandidateId(
          candidateId,
          pageNum,
          9
        );
        ...
        const filteredJobs = jobsWithAppliedStatus.filter(
          (job) => !job.isApplied
        );

        if (filteredJobs.length === 0 && pageNum === 1) {
          setError("No saved jobs found. Save some jobs to see them here.");
          setHasMore(false);
          setJobsPerPage(0);
          return;
        }
        ...
        setJobs((prevJobs) => {
          const newJobs = reset ? filteredJobs : [...prevJobs, ...filteredJobs];
        ...
        // Update jobsPerPage
        setJobsPerPage(filteredJobs.length || 9);

        // Update pagination
        const totalPages = data.totalPages || 1;
        setHasMore(
          (prev) => prev && pageNum < totalPages && filteredJobs.length > 0
        );
      } catch (error: any) {
        const errorMessage =
