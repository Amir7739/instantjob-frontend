...
export const fetchInitialJobApplications = async (): Promise<JobApplicationsResponse> => {
  try {
    const response: AxiosResponse<JobApplicationsResponse> = await axiosInstance.get("/job-applications/get-job-candidates-details", {
      params: { page: 1 },
    });
    console.log("fetchInitialJobApplications response:", response.data);
    if (!response.data.success) {
      throw new Error(response.data.message || "Failed to fetch initial job applications");
    }
    return {
      ...response.data,
    ...
    throw new Error(error.message || "Error fetching initial job applications");
  }
};

// Fetch more job applications (next 10)
export const fetchMoreJobApplications = async (page: number): Promise<JobApplicationsResponse> => {
  try {
    const response: AxiosResponse<JobApplicationsResponse> = await axiosInstance.get("/job-applications/get-job-candidates-details", {
      params: { page },
    });
    console.log("fetchMoreJobApplications response:", response.data);
    if (!response.data.success) {
      throw new Error(response.data.message || "Failed to fetch more job applications");
    }
    return {
      ...response.data,
      data: response.data.data.map((app) => ({
