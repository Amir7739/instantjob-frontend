...
export const fetchInitialJobs = async (): Promise<JobsResponse> => {
  try {
    const response: AxiosResponse<JobsResponse> = await axiosInstance.get("/jobs/admin-pannel", {
      params: { offset: 0, limit: 10},
    });
    console.log('fetchInitialJobs response:', response.data);
...
export const fetchMoreJobs = async (offset: number, title: string = ''): Promise<JobsResponse> => {
  try {
    const response: AxiosResponse<JobsResponse> = await axiosInstance.get("/jobs/admin-pannel", {
      params: { offset, limit: 10, title },
    });
    console.log('fetchMoreJobs response:', response.data);
    if (!response.data.success) {
...
export const fetchJobById = async (id: string): Promise<Job> => {
    try {
      const response: AxiosResponse<JobsResponse> = await axiosInstance.get(`/jobs/get/${id}`);
  ...
  export const deleteJob = async (id: string): Promise<{ success: boolean; message: string }> => {
    try {
      const response: AxiosResponse<{ success: boolean; message: string }> = await axiosInstance.put(`/jobs/delete/${id}`);
      console.log('deleteJob response:', response.data);
      if (!response.data.success) {
